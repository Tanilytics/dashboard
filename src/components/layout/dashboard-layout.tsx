...
function DateRangeDropdown() {
	...
	const selected =
		dateRangeOptions.find((o) => o.value === dateRange) ?? dateRangeOptions[1];
...
function SiteSwitcher({ collapsed }: { collapsed: boolean }) {
	...
	const getSiteColor = (siteId: string) => {
		const idx =
			siteId.split("").reduce((acc, ch) => acc + ch.charCodeAt(0), 0) %
			siteColors.length;
		...
		<DropdownMenu open={open} onOpenChange={setOpen}>
			<DropdownMenuTrigger
				className={cn(
					...
					collapsed
						? "justify-center p-2"
						: "justify-between gap-2 px-[10px] py-2 text-left",
				)}
...
export function DashboardLayout() {
	...
	const pageTitle =
		navItems.find((item) => item.to === currentPath)?.label || "Overview";
	const showDateRange = [
		"/dashboard",
		"/dashboard/realtime",
		"/dashboard/pages",
	...
	return (
		<div className="flex h-screen overflow-hidden bg-background">
			...
			<Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
				...
				<SheetContent
					side="left"
					className="w-64 bg-card border-r border-border p-0 flex flex-col"
				>
					<div className="p-5 pb-4">
						<Link to="/" className="flex items-center gap-3 text-foreground">
							<TanilyticsLogo className="shrink-0 text-primary" />
