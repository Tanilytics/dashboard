...
import { getDateRange, usePages } from "#/lib/queries";
...
function PagesPage() {
	...
	const [sortKey, setSortKey] = useState<
		"views" | "uniqueVisitors" | "uniqueSessions" | "avgTimeOnPageSeconds"
	>("views");
		...
		<Card>
			<CardHeader className="flex flex-row items-center justify-between flex-wrap gap-4">
				...
				<Input
					...
					className="w-70 bg-background border-border max-sm:w-full"
				/>
			</CardHeader>
			<CardContent>
				{isLoading ? (
						...
						<table className="w-full text-sm">
							...
							<tbody>
								{filtered.map((page) => (
									...
									>
										<td
											className="px-4 py-3 truncate max-w-75"
											title={page.url}
										>
											{page.url}
										</td>
										...
										<td className="px-4 py-3">
											{formatTime(page.avgTimeOnPageSeconds)}
										</td>
									</tr>
								))}
							</tbody>
